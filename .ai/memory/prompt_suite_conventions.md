---
name: prompt-suite-conventions
description: The house-style and shared-vocabulary conventions every prompts/system/*.md follows after the PO→Reviewer consistency pass, and which cross-agent invariants are prompt-only (not code-enforced).
metadata:
  type: feedback
---
...
All seven role prompts (`po`, `sa`, `tpm`, `techlead`, `developer`, `qa`, `reviewer`) were aligned to a
single house style. Preserve it when editing any `prompts/system/*.md`. Guardrail: never edit
`prompts/system/*` unless the Human explicitly orders it (CLAUDE.md). Related: [[prompt-language-independence]],
[[agent-contracts]], [[skill-routing-frontmatter]].
...
which caused deadlocks and missed defects in the FSM ([[pipeline-fsm-loops]]).
...
- **Test-pinned literals** in `tests/framework/test_prompts.py` must survive edits (e.g. reviewer
  ...
  [[run-tests-via-wsl]] after any prompt edit.
