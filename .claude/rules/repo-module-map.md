...
`llm.py` (`run_structured_llm` — routes client/model/`max_tokens` by `structured_role_routing` (Gemini `instructor_client` vs Anthropic `get_anthropic_instructor_client` under provider=claude) + `_relocate_jinja_system_messages` — demotes a `{{ }}`/`{% %}`-bearing system message to a user turn so instructor's GenAI guard doesn't reject a config-teaching prompt, e.g. DevOps `${{ secrets.* }}`), `api_retry.py` (`with_api_retry`), `redaction.py` (`redact`),
