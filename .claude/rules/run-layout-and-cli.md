...
- `--provider <name>` → force the WHOLE pipeline onto one LLM provider, overriding the `MODEL_PROVIDER` env: `api`/`google`/`gemini` → Gemini for every role incl. the Developer (a structured Gemini emitter, not the CLI); `claude`/`claude-code`/`cli` → the **subscription Claude Code CLI** for every role (**no API key** — structured roles answer via a one-shot JSON CLI call, the Developer stays agentic); `anthropic`/`claude-api` → the **Anthropic API** for every structured role (needs `ANTHROPIC_API_KEY` + the `anthropic` package; the Developer stays the Claude CLI); omitted/`default` → the mixed routing (Gemini structured roles + Claude-CLI Developer). Applied in `parse_args` via `set_model_provider` BEFORE any role resolves a model or `check_environment` runs; `check_environment` then requires only the keys/binaries the chosen provider exercises. A runtime knob, **not persisted** (like `--budget`) — re-pass it on `--resume`. SSOT: [agent-provider-model-map](agent-provider-model-map.md).
