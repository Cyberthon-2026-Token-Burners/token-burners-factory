...
- **Provider-switch misconfiguration (preflight `sys.exit(1)`, no run dir / no incident)** — the run was
  started with `--provider`/`MODEL_PROVIDER` and `check_environment` exited before any agent ran. **Tell:** a
  `🚨 CRITICAL: MODEL_PROVIDER=anthropic requires ANTHROPIC_API_KEY` line (provider=anthropic, missing key), a
  `🚨 CRITICAL: GEMINI_API_KEY is not set` under a default/gemini provider, a missing `claude` binary under a
  CLI-using provider (default/claude/anthropic), or — only under provider=anthropic if the key check passed
  but the optional dep is absent — a `MODEL_PROVIDER=anthropic requires the 'anthropic' package` `RuntimeError`
  surfacing as repeated structured-call failures. Note `--provider claude` (subscription Claude Code CLI for
  every role) needs NO API key — only the `claude` binary; a structured role under it can still hit the
  Claude session/usage limit (the provider-quota class above, now via `run_claude_cli_oneshot`). The
  pre-flight banner echoes the active provider (`Pre-flight environment check (provider: …)`). All are
  operator/env fixes (set the key, `pip install anthropic`, install/auth the `claude` CLI, or correct the
  `--provider` value), never an agent/prompt edit; provider routing lives in `config.py` (see
  [agent-provider-model-map](../../rules/agent-provider-model-map.md)).
