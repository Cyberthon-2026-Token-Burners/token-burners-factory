...
- **Provider-switch misconfiguration (preflight `sys.exit(1)`, no run dir / no incident)** — the run was
  ...
  `🚨 CRITICAL: MODEL_PROVIDER=claude requires ANTHROPIC_API_KEY` line (claude provider, missing key), a
  `🚨 CRITICAL: GEMINI_API_KEY is not set` under a default/gemini provider, or — only if the key check passed
  but the optional dep is absent — a `MODEL_PROVIDER=claude requires the 'anthropic' package` `RuntimeError`
  surfacing as repeated structured-call failures. The pre-flight banner echoes the active provider
  (`Pre-flight environment check (provider: …)`). All are operator/env fixes (set the key, `pip install
  anthropic`, or correct the `--provider` value), never an agent/prompt edit; provider routing itself lives in
  `config.py` (see [agent-provider-model-map](../../rules/agent-provider-model-map.md)).
