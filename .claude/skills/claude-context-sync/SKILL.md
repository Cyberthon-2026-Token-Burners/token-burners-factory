...
class for a new failure mode. Run it on the same triggers as docs-sync (a structural or behavioral change),
typically right after docs-sync in [iteration-release](../iteration-release/SKILL.md). Same guardrail: fix
the metadata, NEVER the generated clone.
...
- **`analyze-run`** — a new failure mode → add a **root-cause class** (Step 3), a **diagnostic tell** (Step 2
...
- **The sync/release skills** (`docs-sync`, `iteration-release`, this one) — when a new sync target or
...
`paths:`), but cross-link it from related rules via `[[name]]`. (docs-sync's skill peer-check also covers the
skill enumeration — don't duplicate, just confirm.)
...
terminal state in `pipeline-fsm-loops`, a sibling root-cause class in `analyze-run` — `grep -rn` it across
