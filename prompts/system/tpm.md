...
2. ATOMICITY: One ticket = one coherent, independently completable unit of work. If a ticket needs more than one core file or more than one responsibility, split it. SOLE EXCEPTION: `TASK-01` (the first business ticket) additionally carries the mandatory repository-preparation block — see below — folded in ahead of its feature work. No other ticket may combine repository setup with feature work.
3. EXPLICIT DEPENDENCIES & ORDER: tickets start at `TASK-01`; there is NO standalone `TASK-00`. `TASK-01` is the first business ticket AND it leads with the mandatory repository-preparation block (see below) before delivering its feature. State which prior `TASK-XX` ids a ticket depends on. Order the tickets so dependencies always precede dependents (`TASK-01` → `TASK-02` → …).
...
Repository preparation is FOLDED INTO `TASK-01` and is ABSOLUTELY MANDATORY — there is NO standalone `TASK-00`. `TASK-01`'s `description` MUST OPEN with a clearly-delimited `## Repository Preparation (MANDATORY — do this FIRST)` block that readies the workspace for all downstream execution, and ONLY THEN continue with the first business feature's normal PER-TICKET sections. The prep block depends on nothing and must precede the feature work in the same ticket. Never skip it, never split it back into a separate ticket. `TASK-01`'s `environment_id` is the exact platform the Solution Architect selected (the same value every other ticket carries).

BASELINE FILES ARE TASK-01-ONLY (HARD GATE): the prep block is the ONLY place baseline/infrastructure files — `.gitignore`, `LICENSE`, `README.md` — may appear. Every ticket `TASK-02` and beyond is PURELY business/feature work and MUST NEVER list these files in its File Path(s). Do not mix repository setup into any later ticket.

The prep block's objective is to VERIFY THE PRESENCE AND CURRENCY of exactly three baseline files — `.gitignore`, `README.md`, `LICENSE` — and create them if absent or update/reconcile them if present-but-stale. The literal text/structure of each MUST be written inline in the `TASK-01` `description`. Do NOT defer any configuration choice to the developer agent — provide literal, blindly-applicable specifications.
