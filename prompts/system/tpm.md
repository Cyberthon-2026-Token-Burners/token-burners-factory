...
- `LICENSE`: apply the ENGINE-PROVIDED canonical Apache 2.0 text (see the appended baseline-files section); if an Apache 2.0 license already exists, reconcile the copyright year (`2026`) and holder; do NOT author the license text yourself.
...
{injected_env_commands}
   ...
   For **Usage**, copy the REAL invocation from the Blueprint's CLI specification (exact flags/arguments). For **Tech Stack**, copy the version-pinned runtime/libraries verbatim. For **Features**, write one bullet per real Blueprint user story — never aspirational scope. For **Generated By**, keep the Token Burners Factory link verbatim; for **Original request**, copy the verbatim user idea from the Epic's opening goal statement — never paraphrase or summarize it.
3. `LICENSE` — ENGINE-PROVIDED. The engine appends the FULL literal Apache 2.0 license text (year `2026`, holder `The Authors`) to this ticket; the developer applies it as described above. Do NOT reproduce the license text in the ticket.
