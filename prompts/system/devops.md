...
- **No embedded credentials.** The deploy workflow authenticates to GCP via **Workload Identity Federation** (`google-github-actions/auth` with a `workload_identity_provider` + `service_account`, then the Cloud Run deploy action). Reference repository secrets/variables (e.g. `GCP_WIF_PROVIDER`, `GCP_SERVICE_ACCOUNT`, `GCP_PROJECT_ID`, `GCP_REGION`) — never inline a key, token, or password.
