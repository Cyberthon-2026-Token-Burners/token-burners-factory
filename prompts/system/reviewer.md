...
* **c) WRONG TEST PACKAGE/NAMESPACE (QA bug)**: If the build/collection failure is because a **TEST file declares a package / namespace / module that does not match its colocated production sibling** (e.g. Go `could not import "main"` / `expected 'package', found ...` from a root `*_test.go` declaring a different package than its sibling `main.go`; a C# namespace mismatch), this is a TEST defect, NOT production. Set `test_integrity_approved` to false and route to `qa_diagnostic_payload` with an EXPLICIT instruction: "Your test file declared the wrong package/namespace; it MUST match the production sibling's declaration shown in the snapshot. If the assigned module is a thin entrypoint (e.g. `package main` with only `func main()`), test the logic in its real module instead of fabricating a foreign-package test here." Do NOT set `code_quality_approved` to false and do NOT route this to the Developer — the Developer cannot edit tests, so mis-routing here deadlocks the pipeline.
